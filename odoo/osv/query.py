...
class Query(object):
        ...
        if rhs_alias not in self._joins:
            ...
            if extra:
                condition = condition + " AND " + \
                    extra.format(lhs=lhs_alias, rhs=rhs_alias)
                condition_params = list(extra_params)
            if kind:
                self._joins[rhs_alias] = (
                    kind, rhs_table, condition, condition_params)
            else:
    ...
    def add_join(self, connection, implicit=True, outer=False, extra=None, extra_params=()):
        ...
        rhs_alias = self._join(kind, lhs_alias, lhs_column,
                               rhs_table, rhs_column, link, extra, extra_params)
