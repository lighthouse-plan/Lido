...
class TestSyncGoogle2Odoo(TestSyncGoogle):
    ...
    def test_new_google_event(self):
        ...
        self.assertEqual(html2plaintext(event.description),
        ...
        self.assertEqual('山田 太郎', admin_attendee.partner_id.name)
    ...
    def test_recurrence_datetime(self):
        ...
        self.assertEqual(events[0].google_id,
                         '%s_20200106T170000Z' % recurrence_id)
        self.assertEqual(events[1].google_id,
                         '%s_20200113T170000Z' % recurrence_id)
        self.assertEqual(events[2].google_id,
                         '%s_20200120T170000Z' % recurrence_id)
        self.assertGoogleAPINotCalled()
    ...
    def test_recurrence_outlier(self):
        ...
        self.assertEqual(
            len(events), 3, "it should have created a recurrence with 3 events")
        self.assertEqual(events[0].start_date, date(2020, 1, 6))
        self.assertEqual(events[1].start_date, date(
            2020, 1, 18), "It should not be in sync with the recurrence")
        self.assertEqual(events[2].start_date, date(2020, 1, 20))
        self.assertGoogleAPINotCalled()
    ...
    def test_recurrence_deleted(self):
        ...
        self.assertFalse(recurrence.exists(),
    ...
    def test_simple_event_into_recurrency(self):
        ...
        event = self.env['calendar.event'].search(
            [('google_id', '=', values.get('id'))])
        self.assertFalse(
            event.exists(), "The old event should not exits anymore")
        self.assertGoogleAPINotCalled()
    ...
    def test_new_google_notifications(self):
        ...
        self.env['calendar.event']._sync_google2odoo(GoogleEvent([values]))
        triggers_after = self.env['ir.cron.trigger'].search(
            [('cron_id', '=', cron_id)])
        new_triggers = triggers_after - triggers_before
        self.assertFalse(
            new_triggers, "The event should not be created with triggers.")
