...
class TestImageUploadProgress(odoo.tests.HttpCase):
    ...
    def test_01_image_upload_progress(self):
        self.start_tour("/test_image_progress",
                        'test_image_upload_progress', login="admin")
    ...
    def test_02_image_upload_progress_unsplash(self):
        ...
        Web_Unsplash.fetch_unsplash_images = http.route(
            "/web_unsplash/fetch_images", type='json', auth="user")(fetch_unsplash_images)
        ...
        Web_Editor.media_library_search = http.route(
            ['/web_editor/media_library_search'], type='json', auth="user", website=True)(media_library_search)
